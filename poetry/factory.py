...
class Factory:
    ...
    def create_poetry(
        self, io, cwd=None, disable_plugins=False
    ):  # type: (IO, Optional[Path], bool) -> Poetry
        if cwd is None:
            cwd = Path.cwd()
        ...
        poetry = Poetry(poetry_file, local_config, package, locker, config, auth_config)

        # Configuring sources
        pool = Pool()
        for source in local_config.get("source", []):
            repository = self.create_legacy_repository(source, auth_config)
            pool.add_repository(
                repository,
        ...
        if not pool.has_default():
            pool.add_repository(PyPiRepository(), True)
