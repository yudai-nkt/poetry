...
def test_builder_find_excluded_files(mocker):
    ...
    builder = Builder(
        Factory().create_poetry(
            NullIO(), Path(__file__).parent / "fixtures" / "complete"
        ),
        NullEnv(),
        NullIO(),
    )
...
def test_builder_find_case_sensitive_excluded_files(mocker):
    ...
    builder = Builder(
        Factory().create_poetry(
            NullIO(), Path(__file__).parent / "fixtures" / "case_sensitive_exclusions"
        ),
        NullEnv(),
        NullIO(),
    )
...
def test_metadata_homepage_default():
    builder = Builder(
        Factory().create_poetry(
            NullIO(), Path(__file__).parent / "fixtures" / "simple_version"
        ),
        NullEnv(),
        NullIO(),
    )
...
def test_metadata_with_vcs_dependencies():
    builder = Builder(
        Factory().create_poetry(
            NullIO(), Path(__file__).parent / "fixtures" / "with_vcs_dependency"
        ),
        NullEnv(),
        NullIO(),
    )
