...
def test_wheel_c_extension():
    ...
    builder = CompleteBuilder(
        Factory().create_poetry(NullIO(), module_path), NullEnv(execute=True), NullIO()
    )
    builder.build()
...
def test_complete_no_vcs():
    ...
    builder = CompleteBuilder(
        Factory().create_poetry(NullIO(), temporary_dir),
...
def test_package_with_include(mocker):
    ...
    builder = CompleteBuilder(
        Factory().create_poetry(NullIO(), module_path), NullEnv(), NullIO()
    )
    builder.build()
